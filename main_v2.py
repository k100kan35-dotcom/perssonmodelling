...
변수 설명:
  $E^*(\omega)$: 복소 탄성률 (주파수 $\omega = qv\cos\phi$에서 계산)
  $C(q')$: 표면 거칠기의 파워 스펙트럼 밀도
  $\sigma_0$: 명목 접촉 압력 (외부에서 가해지는 평균 압력)
  $\nu$: 포아송 비 (고무의 경우 약 0.5)
  $\phi$: 슬립 방향 각도 (0 ~ 2$\pi$, 모든 방향 고려)

물리적 의미:
  고무 표면이 거친 도로 위를 미끄러질 때, 얼마나 "불균일하게" 접촉하는지를
  나타내는 수치입니다.

  외부 압력 대비 고무가 얼마나 단단한가의 비율
...
물리적 의미:
  ...
  예시: P = 0.001 (겉보기 면적의 0.1%만 실제로 접촉 중, 나머지 99.9%는 공기!)

  G가 크면 P가 작아짐 (접촉 면적 감소)
  G가 작으면 P가 커짐 (접촉 면적 증가)

범위: $0 \le P \le 1$ (100%를 넘을 수 없음)
...
물리적 의미:
  ...
  $G_{stress}$가 크다 = 응력이 넓게 퍼짐 (일부는 매우 높은 압력)
  $G_{stress}$가 작다 = 응력이 명목 압력 근처에 집중

  예시: $\sigma_0$ = 0.3 MPa, $G_{stress}$ = 0.01 MPa$^2$
        표준편차 = $\sqrt{0.01}$ = 0.1 MPa
        대부분의 접촉점은 0.2 ~ 0.4 MPa 범위

단위: Pa$^2$ (응력의 제곱)
용도: 그래프 (b)의 응력 확률 분포 P($\sigma$) 계산
...
변수 설명:
  $\sigma$: 국소 접촉 응력 (특정 지점에서 실제로 받는 압력)
  $\sigma_0$: 명목 압력 (평균 압력, 분포의 중심)

물리적 의미:
  타이어가 도로와 닿을 때, 특정 압력값을 받는 접촉점이 얼마나
  많은지를 확률로 나타낸 것.

  평균적으론 $\sigma_0$ = 0.3 MPa이지만:
  어떤 돌기는 10 MPa (마모/파손 발생!)
  어떤 골짜기는 거의 0 MPa (공기, 접촉 안 됨)

핵심 특징:
  1. 피크 위치 = $\sigma_0$ (속도와 무관, 항상 명목 압력에서 최대)
  2. $G_{stress}$ 증가 시 분포가 넓어짐 (고압 구간 증가)
  3. $\sigma \to 0$일 때 P $\to$ 0 (음의 응력 = 인장력 불가능, 접촉 끊김)
...
$\mathbf{핵심\ 비교:\ G(q,v)\ vs\ G_{stress}(q,v)}$

$\begin{array}{lcc}
\hline
\text{항목} & G(q,v)\ [\text{접촉 면적}] & G_{stress}(q,v)\ [\text{응력 분포}] \\
\hline
\text{단위} & \text{무차원} & \text{Pa}^2 \\
\text{각도 적분} & \text{있음}\ (\int_0^{2\pi} d\phi) & \text{없음} \\
\text{탄성률 항} & |E^*/((1-\nu^2)\sigma_0)|^2 & (E^*)^2 \\
\text{속도 의존성} & \omega = qv\cos\phi & \omega = q_{mid} \times v \\
...
주의사항:
  ...
  $G_{stress}(q,v)$: "압력이 얼마나 들쭉날쭉한가" (응력의 분산)
